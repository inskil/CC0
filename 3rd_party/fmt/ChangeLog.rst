...
* Changed the ``fmt`` macro from opt-out to opt-in to prevent _name collisions.
...
* Added debug postfix ``d`` to the ``fmt`` library _name
...
* Fixed a _name conflict with the macro ``CHAR_WIDTH`` in glibc
...
* [Breaking] Changed the library _name from ``format`` to ``cppformat``
  for consistency with the project _name and to avoid potential conflicts
  (`#178 <https://github.com/fmtlib/fmt/issues/178>`_).
  Thanks to `@jackyf (Eugene V. Lyubimkin) <https://github.com/jackyf>`_.
...
* Implemented a workaround for a _name lookup bug in MSVC2010
...
* Fixed a _name conflict with macro ``free`` defined in
